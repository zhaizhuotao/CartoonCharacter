...
Shader "MagicTavern/Hair_Anisotropy2Pass" 
{
	Properties 
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed3 worldLightDir = normalize(UnityWorldSpaceLightDir(worldPos));
                diffuseColor *= saturate(lerp(0.25, 1.0, dot(worldNormal, worldLightDir)));
                ...
                //half3 t1 = ShiftTangent(worldBinormal, worldNormal, _PrimaryShift);
                //half3 t2 = ShiftTangent(worldBinormal, worldNormal, _SecondaryShift);
				//计算高光强度
